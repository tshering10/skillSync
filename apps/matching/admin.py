...
class MatchResultAdmin(admin.ModelAdmin):
    list_display = ('match_score_display', 'resume_filename', 'job_title', 'matched_count', 'missing_count', 'created_at')
    list_filter = ('created_at',)
    search_fields = ('resume__original_filename', 'job_description__title', 'job_description__company')
    readonly_fields = ('id', 'resume', 'job_description', 'match_score', 'matched_skills', 'missing_skills', 'explanation', 'created_at')
    ordering = ('-created_at',)

    fieldsets = (
        ('Match Pair', {'fields': ('id', 'resume', 'job_description')}),
        ('Score & Results', {'fields': ('match_score', 'explanation')}),
        ('Skill Breakdown', {'fields': ('matched_skills', 'missing_skills')}),
